...
/// <summary>
/// Implementation of database service with in-memory storage
/// </summary>
public class InMemoryDatabaseService : IDatabaseService
{
    ...
    public InMemoryDatabaseService(string baseUrl)
    {
        _baseUrl = baseUrl;
    }
        ...
        var result = new ModuleList
        {
            Modules = modules.Select(m => new ModuleListItem
            {
                ...
                Versions = _modules.Values
                    ...
                    .ToList(),
                DownloadUrl = $"{_baseUrl}/v1/modules/{m.Namespace}/{m.Name}/{m.Provider}/{m.Version}/download"
            }).ToList(),
    ...
    public Task<Module?> GetModuleAsync(string @namespace, string name, string provider, string version)
    {
        ...
        if (!_modules.TryGetValue(key, out var storage))
        {
    ...
    public Task<ModuleStorage?> GetModuleStorageAsync(string @namespace, string name, string provider, string version)
    {
        ...
        if (!_modules.TryGetValue(key, out var storage))
        {
            return Task.FromResult<ModuleStorage?>(null);
        }
