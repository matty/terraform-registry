...
public interface IVcsSourceService
{
    ...
    Task<VcsSource> CreateVcsSourceAsync(string userId, string moduleNamespace, string name, string provider, string repoOwner, string repoName, Guid connectionId);
    ...
    Task<VcsSource?> GetByModuleAsync(string moduleNamespace, string name, string provider);
    Task<VcsSource?> UpdateSyncStateAsync(Guid id, string status, string? lastPublishedVersion, string? errorMessage);
}
